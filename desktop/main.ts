...
import { loadFlowConfig } from "../src/config/config-loader.js";
...
async function startDashboardServer(flowRoot: string): Promise<number> {
  const config = await loadFlowConfig({ projectRoot: flowRoot });
  const dashboardState = new DashboardState({ repoRoot: flowRoot });
