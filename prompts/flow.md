...
- Use `flow_backlog` when the user asks to pull more work from the
  configured issue backlog into the active queue; `flow_queue` only
  shows active work.
...
- Keep workflow actions inside the configured MCP surface; do not ask a person
  to use raw provider UIs unless Flow reports a hard blocker.
